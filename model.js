...
class Model {
    ...
    categoricalCrossEntropy() {
        let y_pred = globalVariables.layerCount - 1
        y_pred = globalVariables.output[y_pred]
        console.log(y_pred)

        const loss = -math.sum(math.multiply(globalVariables.y_true, math.log(y_pred)))
...
model.addLayer({ inputDim: inputDim, units: 4, activation: "relu", useBias: true })
model.addLayer({ units: 4, activation: "relu", useBias: true })
